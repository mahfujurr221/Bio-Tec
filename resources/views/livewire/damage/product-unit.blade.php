<div>
    <div class="form-group">
      ...
      <select name="warehouse_id" id="" class="form-control" wire:model="warehouse_id">
      ...
      <div class="form-group">
        ...
        <select name="product_id" class="form-control" data-provide="selectpicker" data-live-search="true" data-size="10" wire:model="selected_product">
          ...
          @foreach (\App\Product::all() as $item)
            <option value="{{ $item->id }}" {{ old("product_id")==$item->id?"SELECTED":"" }}>{{ $item->name." - Stock: ".$item->readable_qty($item->stock($warehouse_id)) }}</option>
          @endforeach
        </select>
