        ...
        <div class="card">
            <div class="row">
                <div class="col-12" style="">
                    <h4 class="card-title" style="display: inline-block;">Update Member</h4>
          ...
          data-target="#edit" id="Add Category">Add Category</a> --}}
            ...
            <form action="{{ route('sales-member.update', $sales_member) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="card-body">
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" name="name" value="{{ old('name', $sales_member->name) }}"
                                class="form-control">
                            @error('name')
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" name="mobile" value="{{ old('mobile', $sales_member->mobile) }}"
                                class="form-control">
                            @error('mobile')
                                <div class="text-danger mt-2">{{ $message }}</div>
                            @enderror
                                ...
                                <select name="warehouse_id" id="warehouseId"
                                    ...
                                    @foreach ($warehouses as $warehouse)
                                        <option value="{{ $warehouse->id }}"
                                            @if ($sales_member->warehouse_id == $warehouse->id) selected @endif>
                                            {{ $warehouse->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group col-12">
                            ...
                            <textarea name="address" id="" cols="30" rows="4" class="form-control">{{ old('address', $sales_member->address) }}</textarea>
                            @error('address')
                                <div class="text-danger mt-2">{{ $message }}</div>
                            @enderror
                        </div>

                        @livewire('member-designation-supiror', ['designation_id' => $sales_member->sales_designation_id, 'supirior_id' => $sales_member->supirior_id, 'sales_member' => $sales_member])
                    </div>
            ...
            if (role == 'admin') {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.each(data, function(key, value) {
                            $('#salesAreaId').append(
                                `<option value="${value.id}" ${value.sales_area_id} == {{ $sales_member->sales_area_id }} ? select :''>                  
                            ${value.name}</option>`);
                        });
                    }
                });
