...
class ReportController extends Controller
{
    ...
    {
        Gate::authorize('target_report');
        if ($request->member_id != null || $request->designation_id != null) {
            if (auth()->user()->hasRole('WarehouseAdmin')) {
                $members = $members = SalesMember::where('warehouse_id', auth()->user()->warehouse_id)->where('id', $request->member_id)->orWhere('sales_designation_id', $request->designation_id)->orderBy('sales_designation_id', 'asc')->paginate(20);
            } else {
                $members = SalesMember::where('id', $request->member_id)->orWhere('sales_designation_id', $request->designation_id)->orderBy('sales_designation_id', 'asc')->paginate(20);
            }
            return view('pages.reports.target_report', compact('members'));
        } else {
            if (auth()->user()->hasRole('WarehouseAdmin')) {
                $members = SalesMember::where('warehouse_id', auth()->user()->warehouse_id)->orderBy('sales_designation_id', 'asc')->paginate(20);
            } else {
                $members = SalesMember::orderBy('sales_designation_id', 'asc')->paginate(20);
            }
            return view('pages.reports.target_report', compact('members'));
        }
    }
    // member_target_report
    public function member_target_report($id)
    {
        // Gate::authorize('member_target_report');
        $supirior = SalesMember::where('id', $id)->groupBy('supirior_id')->first();
        // dd($supirior);
        if ($supirior->designation->name != 'MPO') {
            $members = SalesMember::where('supirior_id', $id)->paginate(20);
            return view('pages.reports.target_report_supirior', compact('members', 'supirior'));
        } else {
            $target = SalesTarget::with('sales_member')->where('sales_member_id', $id)->first();
            if ($target == null) {
                return redirect()->back()->with('error', 'No Target Found');
            }
            $productTargets = SalesTargetItem::where('sales_target_id', $target->id)->paginate(20);
            return view('pages.reports.target_report_salesman', compact('target', 'productTargets'));
        }
    }

    public function target_summary(Request $request)
    {
        // dd('ok');
        if ($request->member_id != null || $request->designation_id != null || $request->area_id != null) {
            if (auth()->user()->hasRole('WarehouseAdmin')) {
                $members = $members = SalesMember::where('warehouse_id', auth()->user()->warehouse_id)->where('id', $request->member_id)->orWhere('sales_designation_id', $request->designation_id)->orWhere('sales_area_id', $request->area_id)->orderBy('sales_designation_id', 'asc')->paginate(20);
            } else {
                $members = SalesMember::where('id', $request->member_id)->orWhere('sales_designation_id', $request->designation_id)->orderBy('sales_designation_id', 'asc')->orWhere('sales_area_id', $request->area_id)->paginate(20);
            }
            return view('pages.reports.target_summary', compact('members'));
        }
        if ($request->area_id != null) {
            if (auth()->user()->hasRole('WarehouseAdmin')) {
                $members = $members = SalesMember::where('warehouse_id', auth()->user()->warehouse_id)->where('area_id', $request->area_id)->orderBy('sales_designation_id', 'asc')->paginate(20);
            } else {
                $members = SalesMember::where('sales_area_id', $request->area_id)->orderBy('sales_designation_id', 'asc')->paginate(20);
            }
            return view('pages.reports.target_summary', compact('members'));
        } else {
            if (auth()->user()->hasRole('WarehouseAdmin')) {
    ...
    // daily invoice 
    public function daily_invoice($id)
    {
        //sales member id
        $member = SalesMember::join('sales_targets', 'sales_members.id', '=', 'sales_targets.sales_member_id')->where('sales_members.id', $id)->select('sales_members.*', 'sales_targets.id as target_id')->first();
        // dd($member->target_id);
        //get today date 
        $today = Carbon::now()->format('Y-m-d');
        $pos = Pos::where('pos.sale_date', $today)->where('pos.sales_member_id', $id)->groupBy('customer_id')->get();
        $targeted_products = SalesTargetItem::where('sales_target_id', $member->target_id)->get();
        return view('pages.reports.daily_invoice', compact('pos', 'member', 'targeted_products'));
    }
