...
Route::prefix('back')->middleware(['auth'])->group(function () {
     ...
     * ******
     */
    Route::match(['get', 'post'], 'add-stock', 'StockController@add_stock')->name('stock.add_stock');
    Route::get('stock/history', 'StockController@history')->name('stock.history');

    Route::match(['get', 'post'], 'transfer-stock', 'StockController@transfer_stock')->name('stock.transfer_stock');
    Route::get('stock/transfer/history', 'StockController@transfer_history')->name('stock.transfer_history');

    Route::get('stock', 'StockController@index')->name('stock.index');
    Route::get('stock/create', 'StockController@create')->name('stock.create');
    Route::get('stock/transfer', 'StockController@transfer_index')->name('stock.transfer_index');
     ...
     * ********
     */
    Route::any('report/target_report', 'ReportController@target_report')->name('target_report');
    Route::any('report/target_summary', 'ReportController@target_summary')->name('target_summary');
    Route::any('report/member_target_report/{id}', 'ReportController@member_target_report')->name('target_report.member');
    Route::any('report/target_report/daily_sale_collection', 'ReportController@daily_sale_collection')->name('target_report.daily_sale_collection');
    Route::any('report/daily_invoice/{id}', 'ReportController@daily_invoice')->name('daily_invoice.invoice');
     ...
     * *********************
     */
    Route::resource('sales-designation', 'SalesDesignationController');
    Route::resource('sales-area', 'SalesAreaController');
    Route::resource('sales-member', 'SalesMemberController');

    Route::post('sales-target/{sales_target}/items/store', 'SalesTargetController@store_item')->name('sales_target.store_item');
    Route::put('sales-target/{sales_target_item}/items/update', 'SalesTargetController@update_item')->name('sales_target.update_item');
    Route::resource('sales-target', 'SalesTargetController');

    Route::get('process-commission', function () {
        Artisan::call('db:seed --class=GenerateCommission');
        session()->flash('success', 'Processed');
        return back();
